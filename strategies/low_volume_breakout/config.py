...
class StrategyConfig:
    ...
    Attributes:
        ...
        low_threshold: 低位阈值（当前价格/250日最高价）
        volume_ratio: 放量倍数（5日均量/20日均量）

        # 数据参数
        data_period: 获取历史数据天数
        min_data_points: 最小数据点数（用于计算指标）
    ...
    # 数据参数
    data_period: int = 300  # 获取历史数据天数
    min_data_points: int = 250  # 最小数据点数（用于计算指标）

    # 技术指标参数
    ma_short: int = 5  # MA5
    ma_mid: int = 20  # MA20
    ma_long: int = 60  # MA60
    ma_trend: int = 250  # MA250
    volume_ma_short: int = 5  # 5日均量
    volume_ma_mid: int = 20  # 20日均量
    volume_ma_long: int = 60  # 60日均量
    high_period: int = 250  # 250日最高价周期
